...
export default function Settings() {
  ...
  return (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 10 }}>
      <Stack spacing={8}>
        <TableRowGroup title="GuildLurk Debug">
          <TableRow
            label="What is this?"
            subLabel="Diagnostic copy that records Discord's actual lurk error"
          />
        </TableRowGroup>

        {storage.lastLurkError ? (
          <TableRowGroup title="Last Lurk Error">
            <RN.Text selectable>{storage.lastLurkError}</RN.Text>
          </TableRowGroup>
        ) : null}
